...
- Provides an inbox for giving feedback which is powered by log matching i.e. that when a new log is received, active tasks are checked to see if their tag selectors match the log. If there is a match, the log is added to the shared inbox. Read more [here](/docs/feedback/log_matching).
