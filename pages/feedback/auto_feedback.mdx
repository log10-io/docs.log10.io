...
## Autofeedback: latent readout algorithm (alpha)

Latent readout is a novel approach to generating feedback developed by Log10 based on the latest research.

This approach has significantly higher accuracy than traditional methods and can be customized to specific datasets.

An example summarization task has been provided to demonstrate the capabilities of the latent readout approach.

Clicking on `Summary grading (latent readout)` creates a summarization task that uses the latent readout algorithm. To generate feedback, log completions with the tag `log10/summary-grading-latent-readout` to your organization.
![Summary onboarding (latent readout) task](/images/docs/autofeedback/onboarding-task-latent-readout.png)

For information on latent readout for your use case, please get in touch with us at support@log10.io