...
- `create`: Allows defining a new feedback task with a specific schema and optional name, instructions and comletion tag selector i.e. tags that should be present in the logs for the feedback task to be matched. Read more in the [log matching](/feedback/log_matching) section.
